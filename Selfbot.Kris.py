...
Bots = "u31ef22df7f538df1d74dc7f756ef1a32"
...
def bot(op):
    try:
        if op.type == 0:
            return
        if op.type == 15:
            kk.sendText(op.param1,cl.getContact(op.param2).displayName + ", Selamat Jalan\ndan jangan lupa selalu bahagia ^_^\nMakasih")
            print "Ada Member Keluar Dari Grup"
        if op.type == 17:
            kk.sendText(op.param1,cl.getContact(op.param2).displayName + ", Selamat Datang di Room Family Ini^_^\nJangan Nakal & Jangan Baper Ya ^_^")
            print "Ada Member Masuk ke Grup"
        if op.type == 19:
            kk.sendText(op.param1,cl.getContact(op.param2).displayName + ", Waduh Dia Kicker :(\nGimana Donk\nSaya Gk Bisa anu Dia")
            print "Ada Kicker"
        if op.type == 32:
...
#--------------------------------------------------------
		...
		try:
		    ...
		    cl.comment(url['result']['posts'][0]['userInfo']['mid'], url['result']['posts'][0]['postInfo']['postId'], "👉ąµţ๏ℓɨЌ€ By RcO-Bot😊\n\n☆º°˚˚☆One Piece Bot☆º°˚˚☆（＾ω＾）\nąµţ๏ℓɨЌ€ by Kris ⭐👈 »»» http://line.me/ti/p/GkwfNjoPDH «««")
